...
const Register = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    workspaceName: '' // Company/workspace name (only if no invitation)
  });
  ...
  useEffect(() => {
    // If invitation token exists, workspace name is not required
    if (invitationToken) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!invitationToken) {
      requiredFields.push('workspaceName');
    }
    ...
    try {
      ...
      if (invitationToken) {
        registerPayload.invitationToken = invitationToken;
      } else {
        // Only include workspaceName if no invitation token
        registerPayload.workspaceName = formData.workspaceName.trim();
      }
      ...
      <div className="register-left">
        <div className="register-form-container">
          ...
          <h2 className="register-form-title">
            {invitationToken ? 'Accept Invitation & Create Account' : 'Create Your Account'}
          </h2>

          {invitationToken && (
            <div className="register-info-banner">
              <p>You've been invited to join a company. Complete your registration below.</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="register-form">
            ...
            >
              {loading ? (
                ...
                invitationToken ? 'Create Account & Join' : 'Create Workspace & Account'
              )}
            </button>
          </form>
